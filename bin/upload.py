...
DEFAULT_REVIEW_SERVER = "codereview.bluedigm.com"
...
class AbstractRpcServer(object):
  ...
  def Send(self, request_path, payload=None,
           ...
           timeout=None,
           extra_headers=None,
           request_password_if_302=True,  # Add(open42):
           **kwargs):
      ...
      while True:
        ...
        try:
          ...
          if tries > 3:
            ...
            #self._Authenticate()
            # Change start (open42): Don't authenticate again, 302 is normal
            if request_password_if_302:
              self._Authenticate()
            else:
              logging.debug("e.code = " + str(e))
              raise
            # Change end(open42)
  ...
  def GetBaseFile(self, filename):
    ...
    if filename in self.renames:
      ...
      if filename not in self.hashes:
        ...
        # Open42(added silent_ok=True so that empty files can be added):
...
def RealMain(argv, data=None):
  ...
  # Add(Open42):
  if options.verbose:
    logging.debug("TEST: " + str(options.server) +
                  " EMAIL:" + str(options.email) +
                  " HOST:" + str(options.host) +
                  " COOKIES:" + str(options.save_cookies) +
                  " ACCT_TYPE:" + str(options.account_type))
    logging.debug("ARGV for upload.py:" + str(argv))
    #sys.exit(0)
  ...
  # Add(open42): Dry run.
  if options.dryrun:
    print "This is only a dry run, exiting now."
    sys.exit(0)
